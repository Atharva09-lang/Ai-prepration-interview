...
Partial failures recorded in the kit's warnings (questions_failed, gap_questions_failed, flashcards_failed, flashcards_empty, uncovered_must_haves)	A thin kit explains why it is thin instead of looking complete; the kit page renders those warnings plus coverage.uncovered_requirement_ids and research.pages_failed	Kits can carry several warning strings to read
No fabricated placeholder content	A failed flashcard step yields an empty section plus a warning, and regenerating a section that returns nothing raises an error instead of emptying it	A failed section stays empty until the user regenerates or adds items by hand
